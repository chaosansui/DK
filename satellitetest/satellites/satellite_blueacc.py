...
def satellite_blueacc_run(time:Time,orbit:Orbit):
    # 创建轨道对象
    blueacc_orbit =orbit
    ...
    # 循环推进轨道并输出信息
    current_time = blueacc_orbit.epoch
    while True:
        # 推进轨道
        blue_orbit = blueacc_orbit.propagate(current_time + time_step_as_timedelta)

        # 获取位置和速度
        r, v = blueacc_orbit.rv()
